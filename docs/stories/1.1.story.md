# Story 1.1: Project Registry & Structure

## Status

Draft

## Executor Assignment

executor: "@dev"
quality_gate: "@architect"
quality_gate_tools: [pattern_validation, schema_review]

## Story

**As a** NEXUS operator,
**I want** to register projects with standardized metadata and structure,
**so that** each project has its own isolated knowledge namespace.

## Acceptance Criteria

1. Comando `*project create {slug}` cria estrutura `docs/knowledge/{slug}/` com `manifest.yaml`
2. Comando `*project list` exibe todos os projetos registrados com status
3. Comando `*project select {slug}` define o projeto ativo (persiste em `.aios/session.yaml`)
4. Comando `*project status` mostra projeto ativo com contagem de substratos, blocos e outputs
5. Projetos `innovatech`, `evidex` e `ipro` são registrados como seed data
6. O `manifest.yaml` inclui campo `visibility: public|private` para separação futura

## 🤖 CodeRabbit Integration

> **CodeRabbit Integration**: Disabled
>
> CodeRabbit CLI is not enabled in `core-config.yaml`.
> Quality validation will use manual review process only.
> To enable, set `coderabbit_integration.enabled: true` in core-config.yaml

## Tasks / Subtasks

- [ ] Task 1: Setup do projeto Node.js (AC: todos)
  - [ ] 1.1 Criar `package.json` conforme arquitetura (ESM, 6 deps, devDeps)
  - [ ] 1.2 Criar `vitest.config.js` com configuração da arquitetura
  - [ ] 1.3 Criar `eslint.config.js` com regras da arquitetura
  - [ ] 1.4 Rodar `npm install`
- [ ] Task 2: Criar shared modules (AC: todos)
  - [ ] 2.1 Criar `src/shared/schemas.js` — schemas Zod: ProjectManifest, SubstrateCategory, SubstrateFrontmatter (importar de arquitetura seção 4)
  - [ ] 2.2 Criar `src/shared/fs-utils.js` — `readWithFrontmatter()`, `writeWithFrontmatter()`, `ensureDir()`
  - [ ] 2.3 Criar `src/shared/paths.js` — path resolver centralizado (`projectDir()`, `substratesDir()`, `blocksDir()`, `contextsDir()`, `outputsDir()`, `sessionFile()`)
  - [ ] 2.4 Criar `src/shared/errors.js` — hierarchy completa: `NexusError`, `ValidationError`, `NotFoundError`, `ProjectNotFoundError`, etc.
  - [ ] 2.5 Criar `src/shared/constants.js` — `MAX_WORD_COUNT`, `FUNNEL_STAGES`, `SUBSTRATE_CATEGORIES`, `VISIBILITY_LEVELS`, theme object
- [ ] Task 3: Implementar Project Registry (AC: 1, 2, 4)
  - [ ] 3.1 Criar `src/project/registry.js` — `createProject(slug, metadata)`, `listProjects()`, `getProject(slug)`
  - [ ] 3.2 Criar `src/project/session.js` — `selectProject(slug)`, `getActiveProject()` via `.aios/session.yaml`
  - [ ] 3.3 Criar `src/project/visibility.js` — `setVisibility(slug, level)`, `getVisibility(slug)`
  - [ ] 3.4 Criar `src/project/index.js` — API pública do módulo
- [ ] Task 4: Implementar CLI commands (AC: 1, 2, 3, 4)
  - [ ] 4.1 Criar `src/cli/project.js` — Commander.js commands: `create`, `list`, `select`, `status`
- [ ] Task 5: Seed data (AC: 5, 6)
  - [ ] 5.1 Criar `docs/knowledge/innovatech/manifest.yaml` com dados reais
  - [ ] 5.2 Criar `docs/knowledge/evidex/manifest.yaml` com dados reais
  - [ ] 5.3 Criar `docs/knowledge/ipro/manifest.yaml` com dados reais
  - [ ] 5.4 Criar subdirs: `substrates/`, `blocks/`, `contexts/`, `outputs/` para cada
- [ ] Task 6: Testes unitários (AC: todos)
  - [ ] 6.1 Criar `tests/shared/schemas.test.js` — validação Zod (válido + inválido)
  - [ ] 6.2 Criar `tests/shared/fs-utils.test.js` — read/write com frontmatter
  - [ ] 6.3 Criar `tests/project/registry.test.js` — CRUD + listagem
  - [ ] 6.4 Criar `tests/project/session.test.js` — select/get active
  - [ ] 6.5 Criar `tests/project/visibility.test.js` — set/get visibility
  - [ ] 6.6 Criar fixtures: `tests/fixtures/projects/test-project/manifest.yaml`, `tests/fixtures/projects/personal-project/manifest.yaml`
- [ ] Task 7: Validação final
  - [ ] 7.1 `npm run lint` passa
  - [ ] 7.2 `npm test` passa com coverage 85%+

## Dev Notes

### Arquitetura Relevante

**Tech Stack** [Source: architecture/nexus-architecture-v1.md#3]:
- Node.js 22, ESM only (`"type": "module"`)
- JavaScript puro (sem TypeScript no MVP)
- 6 deps: commander, fs-extra, glob, gray-matter, js-yaml, zod
- DevDeps: eslint 9.x, vitest 2.x, @vitest/coverage-v8
- Package manager: npm

**Module Pattern** [Source: architecture/nexus-architecture-v1.md#11.2]:
```javascript
// ESM, funções puras, Zod via shared, fs via fs-utils
import yaml from 'js-yaml'
import { readWithFrontmatter } from '../shared/fs-utils.js'
import { paths } from '../shared/paths.js'
import { ValidationError } from '../shared/errors.js'

const MAX_WORD_COUNT = 200

export function publicFunction(param) { /* ... */ }
function internalHelper() { /* ... */ }
```

**ProjectManifest Schema** [Source: architecture/nexus-architecture-v1.md#4.1]:
```javascript
const ProjectManifest = z.object({
  slug: z.string().regex(/^[a-z0-9-]+$/),
  name: z.string().min(1),
  description: z.string().optional(),
  visibility: z.enum(['public', 'private', 'personal']).default('private'),
  market_profile: z.string().default('pt-br-massa'),
  tone: z.string().default('professional'),
  created_at: z.string().datetime(),
  tags: z.array(z.string()).default([]),
  shared_substrates: z.array(z.string()).default([]),
  design_system: z.string().optional(),
})
```

**Project Registry Interfaces** [Source: architecture/nexus-architecture-v1.md#6.4]:
- `createProject(slug, metadata)` → cria diretório + manifest
- `listProjects()` → `ProjectManifest[]`
- `selectProject(slug)` → persiste em `.aios/session.yaml`
- `getActiveProject()` → `ProjectManifest`

**Project Structure** [Source: architecture/nexus-architecture-v1.md#12]:
```
src/project/
├── registry.js        # CRUD de projetos
├── session.js         # Projeto ativo
├── cloner.js          # (Story 4.1)
├── visibility.js      # Controle de visibilidade
└── index.js           # API pública
```

**Shared Modules** [Source: architecture/nexus-architecture-v1.md#11.1]:
```
src/shared/
├── schemas.js         # Schemas Zod centralizados
├── fs-utils.js        # readWithFrontmatter, writeWithFrontmatter
├── paths.js           # Path resolver centralizado
├── constants.js       # Enums, defaults, limites
└── errors.js          # Error types padronizados
```

**Error Hierarchy** [Source: architecture/nexus-architecture-v1.md#18]:
```
NexusError (base)
├── ValidationError (SchemaError, DuplicateIdError, WordCountExceededError)
├── NotFoundError (ProjectNotFoundError, SubstrateNotFoundError, etc.)
├── SecurityError (VisibilityError, SensitiveContentError)
└── ...
```

**Naming Conventions** [Source: architecture/nexus-architecture-v1.md#17.2]:
- Files: kebab-case (`context-generator.js`)
- Functions: camelCase (`generateContext()`)
- Constants: UPPER_SNAKE (`MAX_WORD_COUNT`)
- Schemas: PascalCase (`SubstrateFrontmatter`)
- Errors: PascalCase+Error (`SubstrateNotFoundError`)

**Critical Rules** [Source: architecture/nexus-architecture-v1.md#17.1]:
- ESM Only — `import/export`, zero `require()`
- Schemas in shared/ — Zod centralizado
- Paths via resolver — Nunca string concatenation
- Frontmatter via fs-utils — Nunca gray-matter direto
- Validate before write — Zod parse antes de persistir
- Errors are typed — Classes de errors.js

**Seed Data Existente** — os projetos innovatech, evidex e ipro já existem em `docs/knowledge/` com substratos. Os manifests precisam ser criados no formato padronizado.

**CLI Output Theme** [Source: architecture/nexus-architecture-v1.md#10.1]:
```javascript
export const theme = {
  success: '✅', warning: '⚠️', error: '❌', info: 'ℹ️',
  project: '📁', substrate: '🧬', block: '🧱', context: '📋',
  output: '📄', squad: '👥', campaign: '🚀', pipeline: '⛓️',
}
```

### Testing

[Source: architecture/nexus-architecture-v1.md#16]:
- Framework: Vitest 2.x
- Location: `tests/` (mirrors src/ structure)
- Globals: true (no import needed for describe/it/expect)
- Coverage: v8 provider, threshold 85% lines/functions, 80% branches
- Fixtures: `tests/fixtures/`
- Pattern: `tests/{module}/{file}.test.js`

**Vitest Config:**
```javascript
import { defineConfig } from 'vitest/config'
export default defineConfig({
  test: {
    globals: true,
    environment: 'node',
    include: ['tests/**/*.test.js'],
    coverage: {
      provider: 'v8',
      include: ['src/**/*.js'],
      exclude: ['src/cli/formatters/**'],
      thresholds: { global: { lines: 85, branches: 80, functions: 85 } },
    },
  },
})
```

## Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2026-02-25 | 0.1 | Story criada | River (SM) |

## Dev Agent Record

### Agent Model Used
_To be filled by dev agent_

### Debug Log References
_To be filled by dev agent_

### Completion Notes List
_To be filled by dev agent_

### File List
_To be filled by dev agent_

## QA Results
_To be filled by QA agent_
